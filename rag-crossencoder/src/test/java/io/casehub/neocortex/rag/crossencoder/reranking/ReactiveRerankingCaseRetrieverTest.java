...
class ReactiveRerankingCaseRetrieverTest {
    ...
    void overfetch_usesMaxOfLimitAndPoolSize() {
        ...
        ReactiveCaseRetriever delegate = (q, c, max, f) -> {
            capturedLimit.set(max);
            return Uni.createFrom().item(
                List.of(chunk("a", "d1", 0.5), chunk("b", "d2", 0.6)));
        };

        var reranker = new CrossEncoderReranker(RerankingLogicTest.stubModel(0.3f, 0.8f));
        var retriever = new ReactiveRerankingCaseRetriever(delegate, reranker, config(30));
    ...
    void rerank_sortsOutputByCrossEncoderScore() {
        ReactiveCaseRetriever delegate = (q, c, max, f) -> Uni.createFrom().item(List.of(
    ...
    void alreadyReranked_passesThrough() {
        ...
        ReactiveCaseRetriever delegate = (q, c, max, f) -> Uni.createFrom().item(stamped);

        var retriever = new ReactiveRerankingCaseRetriever(delegate, null, config(30));
    ...
    void stampsOutputToPreventBridgeReentry() {
        ReactiveCaseRetriever delegate = (q, c, max, f) ->
            Uni.createFrom().item(List.of(chunk("a", "d1", 0.5)));
