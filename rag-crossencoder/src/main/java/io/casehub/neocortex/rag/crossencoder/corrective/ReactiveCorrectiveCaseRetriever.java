    ...
    private Uni<List<RetrievedChunk>> evaluateAndCorrect(
        ...
        return Uni.createFrom().item(() -> {
                ...
                if (evaluator instanceof CrossEncoderRelevanceEvaluator ceEval) {
                    var scored = ceEval.evaluateBatchWithScores(
                        query.text(), contents);
                ...
                return CragEvaluationLogic.gradeChunks(gradedInput, grades);
            })
                ...
                return delegate.retrieve(query, corpus, expandedLimit, filter)
                    .onItem().transformToUni(expandedChunks -> {
                        ...
                        return Uni.createFrom().item(() -> {
                                ...
                                if (evaluator instanceof
                                        CrossEncoderRelevanceEvaluator ceEval) {
                                    var scored =
                                        ceEval.evaluateBatchWithScores(
                                            query.text(), newContents);
                                    newGrades = scored.stream()
                                        .map(ScoredGrade::grade).toList();
                                    float[] newScores = extractScores(scored);
                                    newGradedInput = RerankingLogic.attachScores(
                                        newChunks, newScores);
                                } else {
                                    newGrades = evaluator.evaluateBatch(
                                        query.text(), newContents);
                                }

                                return CragEvaluationLogic.gradeChunks(
