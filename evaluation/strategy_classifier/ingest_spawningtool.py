...
from evaluation.strategy_classifier.sc2egset_extractor import UPGRADE_IDX, N_UPGRADES
...
ST_UPGRADE_MAP = {
    ...
    "Resonating Glaives": "AdeptPiercingAttack",
    "Moebius Reactor": "PersonalCloaking",
}
...
def extract_spawningtool_build_order(parsed: dict, player_id: str) -> Tuple[List[Dict], List[Dict]]:
    """Convert spawningtool parsed replay to build-order + upgrade list.

    Returns (build_order, upgrade_events). Build order contains buildings
    and units; upgrade_events contains researched upgrades mapped to
    SC2EGSet internal names.
    """
    ...
    for entry in raw_orders:
        ...
        if name in _ALL_TRACKABLE:
            entry_type = "building" if name in _BUILDINGS_SET else "unit"
            build_order.append({"type": entry_type, "name": name, "minute": minute})
        elif name in ST_UPGRADE_MAP:
            internal_name = ST_UPGRADE_MAP[name]
            upgrade_events.append({"name": internal_name, "minute": minute})

    return sorted(build_order, key=lambda x: x["minute"]), sorted(upgrade_events, key=lambda x: x["minute"])
...
def parse_and_label_replays(
    ...
    for replay_path in replay_files:
        ...
        for observer_id, opponent_id in [(player_ids[0], player_ids[1]),
            ...
            opponent_build, opponent_upgrades = extract_spawningtool_build_order(parsed, opponent_id)
            observer_build, observer_upgrades = extract_spawningtool_build_order(parsed, observer_id)
            ...
            if label is not None:
                labelled[matchup].append((opponent_build, opponent_upgrades, observer_build, observer_upgrades, label))
...
def _build_order_to_features(
    ...
    duration: int = 300,
    upgrade_events: List[Dict] = None,
) -> np.ndarray:
    """Convert a build order into per-second feature arrays.

    Populates building counts, unit counts, and upgrade flags at each second.
    """
    ...
    upgrade_flags = np.zeros(N_UPGRADES, dtype=np.float32)

    sorted_bo = sorted(build_order, key=lambda x: x["minute"])
    bo_idx = 0

    sorted_upgrades = sorted(upgrade_events or [], key=lambda x: x["minute"])
    up_idx = 0
    ...
    for sec in range(duration):
        ...
        while up_idx < len(sorted_upgrades) and sorted_upgrades[up_idx]["minute"] <= current_minute:
            uname = sorted_upgrades[up_idx]["name"]
            if uname in UPGRADE_IDX:
                upgrade_flags[UPGRADE_IDX[uname]] = 1.0
            up_idx += 1

        features[sec, :N_BUILDINGS] = building_counts
        features[sec, N_BUILDINGS:N_BUILDINGS + N_UNITS] = unit_counts
        up_offset = N_BUILDINGS + N_UNITS + N_STATS
        features[sec, up_offset:up_offset + N_UPGRADES] = upgrade_flags
...
def ingest_spawningtool(
    ...
    for matchup in MATCHUPS:
        ...
        for replay_id, (opponent_build, opponent_upgrades, observer_build, observer_upgrades, label) in enumerate(entries):
            if label not in arch_to_idx:
                continue
            label_idx = arch_to_idx[label]

            duration = 300
            opp_feat = _build_order_to_features(opponent_build, duration, opponent_upgrades)
            own_feat = _build_order_to_features(observer_build, duration, observer_upgrades)
